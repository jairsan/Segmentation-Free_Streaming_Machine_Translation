...
if [ "$truecase" = true ]; then
    cat $file_prefix.$lang | $moses_scripts/tokenizer/normalize-punctuation.perl -l $lang | $moses_scripts/tokenizer/tokenizer.perl -a -l $lang -protected /home/jiranzo/trabajo/git/nmt-scripts/tokenizer_protected_patterns | $moses_scripts/recaser/truecase.perl -model $CORPUS_FOLDER/$tc_model.$lang | $SUBWORD_FOLDER/apply_bpe.py -c $FOLDER/$NAME.codes --vocabulary $FOLDER/$NAME.vocab.$lang --vocabulary-threshold 50 
else
    cat $file_prefix.$lang | $moses_scripts/tokenizer/normalize-punctuation.perl -l $lang | $moses_scripts/tokenizer/tokenizer.perl -a -l $lang -protected /home/jiranzo/trabajo/git/nmt-scripts/tokenizer_protected_patterns | $moses_scripts/tokenizer/lowercase.perl | $SUBWORD_FOLDER/apply_bpe.py -c $FOLDER/$NAME.codes --vocabulary $FOLDER/$NAME.vocab.$lang --vocabulary-threshold 50 
