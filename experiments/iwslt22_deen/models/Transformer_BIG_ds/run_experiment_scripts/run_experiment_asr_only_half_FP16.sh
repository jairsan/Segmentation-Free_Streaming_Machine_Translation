...
qsubmit -n p$RUN$CORPUS -m 24 ./preprocess_corpus_ASR_only_half.sh
...
qsubmit -n t$RUN$CORPUS -w bin$RUN$CORPUS -gmem $GPU_MEM -gcards 1 -m 16 -a pinxo,panxo.dsic.upv.es,riddler.dsic.upv.es,sephiroth,gozer1,gozer2,estelles ./train_model_"$TOOLKIT"_"$MODEL_CONFIG"_FP16.sh
