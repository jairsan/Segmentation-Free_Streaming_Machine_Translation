...
hypo_folder=$hypo_root_folder/$set_name.ds_oracle."$segmenter_future_window"_hist"$history_size"_k$k
...
scores=$(
cat tmp.hypo | sacrebleu tmp.ref --language-pair de-en | python3 -c "import sys, json; print(json.load(sys.stdin)['score'])"
)
rm -f tmp.hypo tmp.ref
